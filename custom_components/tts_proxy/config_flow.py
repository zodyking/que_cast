...
class TTSProxyConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        ...
        try:
            json.loads((user_input.get(CONF_TTS_DEFAULT_OPTIONS) or "{}").strip() or "{}")
        except Exception:
            return self.async_show_form(step_id="user", data_schema=STEP_SCHEMA, errors={"base": "invalid_default_options_json"})
