...
class TTSQueue:
    """Single queue/worker for one proxy instance (one config entry)."""
    ...
    async def _worker(self) -> None:
        while not self._stopped.is_set():
            ...
            try:
                ...
                self.hass.logger.exception("tts_proxy: error handling item on %s: %s", self.entry_id, e)
            finally:
                self._queue.task_done()
    ...
    async def _handle_item(self, item: QueueItem) -> None:
        ...
        if item.volume_override is not None:
            ...
            volume = self.conf["night_volume"] if self._in_quiet_hours() else self.conf["day_volume"]
        ...
        # Wait done (state or timer)
        await self._wait_until_idle(target, item.message)

        # Restore after (for ducking)
            ...
            for ent in targets:
                ...
                try:
                    await self.hass.services.async_call(
                        ...
                        {"entity_id": ent, "volume_level": self.conf.get("duck_volume", 0.15)},
                        blocking=False
                    )
                except Exception:
                    pass
            ...
            for ent, prev in self._duck_restore.items():
                try:
                    if prev is not None:
                        await self.hass.services.async_call(
                            "media_player", "volume_set",
                            {"entity_id": ent, "volume_level": max(0.0, min(1.0, prev))},
                            blocking=False
                        )
                except Exception:
                    pass
    ...
    async def _wait_until_idle(self, target: str, message: str) -> None:
